...
function getWeekOfYear(date: Date): number {
	...
	const diff =
		(date.getTime() - start.getTime() + start.getTimezoneOffset() * 60000) /
		86400000
	...
	async ({ step }) => {
		...
		for (const userId in cutsByUser) {
			...
			await step.run(`trigger-novu-event-for-${userId}`, async () => {
				console.log(
					`Triggering Novu event for user ${userId} for ${userData.cuts} cuts`,
				)
				await triggerNovuEvent(
					'cut-reminder',
					userData.novu_subscriber_id,
					{
						services: { amount: userData.cuts },
...
const snowfallCheck = inngest.createFunction(
	...
	async ({ step }) => {
		...
		const snowClientsByUser: Record<
			string,
			{ novu_subscriber_id: string; services: number; address: string }
			...
			if (Object.hasOwn(snowClientsByUser, userId)) {
				...
				await step.run(`trigger-novu-event-for-${userId}`, async () => {
					if (
						...
						if (snowing) {
							console.log(
								`Snowfall predicted for user ${userId}. Triggering notification.`,
							)
							await triggerNovuEvent(
								'snow-reminder',
								userData.novu_subscriber_id,
								{
									services: { amount: userData.services },
									date: now.toISOString(),
								},
							)
						}
						...
						console.error(
							`Could not get geocode for user ${userId}: ${geocodeResult ? geocodeResult.error : 'Unknown error'}`,
						)
					}
				})
			}
