...
export async function handleInvoiceSent(invoice: Stripe.Invoice, orgId: string) {
  ...
  try {
    ...
    if (!customerId || amountDue === undefined || !invoiceId) {
      console.error(`Missing customer ID, amount due, or invoice ID for invoice.sent event: ${invoice.id}`);
      return;
    }

    const clientId = await fetchClientIdByStripeCustomerId(customerId, orgId);

    if (!clientId) {
      console.error(`Client not found for Stripe Customer ID: ${customerId} in organization: ${orgId}`);
      return;
    }
