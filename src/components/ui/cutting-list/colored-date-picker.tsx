...
export const ColoredDatePicker = () => {
    ...
    const getWeekColorClass = (date: Date) => {
        ...
        switch (week) {
            ...
            case 0: return 'bg-yellow-200'; // case 0 is week 4
            default: return '';
        }
    };
                ...
                <div className="origin-top-right absolute right-0 mt-2 w-80 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                    <div className="p-2">
                        <div className="flex justify-between items-center mb-2">
                            <button onClick={() => changeMonth(-1)} className="px-2 py-1 rounded-md hover:bg-gray-100">&lt;</button>
                            <div className="text-lg font-semibold">{date.toLocaleString('default', { month: 'long', year: 'numeric' })}</div>
                            <button onClick={() => changeMonth(1)} className="px-2 py-1 rounded-md hover:bg-gray-100">&gt;</button>
                        </div>
                        <div className="grid grid-cols-7 gap-1 text-center">
                            ...
                            {daysInMonth.map(({ day, isCurrentMonth }, index) => {
                                ...
                                const colorClass = isCurrentMonth ? getWeekColorClass(currentDate) : 'bg-gray-100';
                                return (
                                    <button
                                        key={index}
