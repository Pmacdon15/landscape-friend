...
const VAPID_KEY =
	'BPFSqTStA7Mj1cwUo71zL-1oCgTz6ap4DGGRzEzFpHzA_MYIke8WhKiiHnwg0YBut0Yg3ruXouTNfOvWL3apin4'
...
export default function FCMProvider({
	...
	const [permissionStatus, setPermissionStatus] = useState<
		NotificationPermission | 'not-supported' | 'default'
	>('default')
	...
	const requestNotificationPermissionAndToken = useCallback(async () => {
		...
		try {
			...
			if (permission === 'granted') {
				...
				const currentToken = await getToken(messagingInstance.current, {
					vapidKey: VAPID_KEY,
				})
				...
				if (currentToken && user?.id) {
					...
					console.warn(
						'No registration token available. This may happen if the browser is closed or permission is denied.',
					)
				}
			} else {
		...
		if (
			...
			try {
				...
				onMessage(messagingInstance.current, (payload) => {
					console.log(
						'[FCMProvider] Received foreground message:',
						payload,
					)
					alert(
				...
				if (initialPermission === 'granted') {
					if (messagingInstance.current) {
						getToken(messagingInstance.current, {
							vapidKey: VAPID_KEY,
						})
							.then((currentToken) => {
								if (currentToken && user?.id) {
							...
							.catch((err) => {
								console.error(
									'Error retrieving FCM token on load:',
									err,
								)
							})
							.finally(() => {
