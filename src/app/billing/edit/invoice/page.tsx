...
export default async function EditInvoicePage({
  ...
  const invoiceId = searchParams?.invoice as string;
  
  if (!invoiceId) {
    return <FormContainer><FormHeader text={'Error: Invoice ID not found.'} /></FormContainer>;
  }

  const invoice = await getInvoiceDAL(invoiceId);

  if (!invoice) {
    return <FormContainer><FormHeader text={'Error: Invoice not found.'} /></FormContainer>;
  }
    ...
    <FormContainer>
      <FillFormContainer>
        ...
        <div className="p-4 border rounded-md shadow-sm">
          <EditInvoiceForm organizationId={organizationId} invoice={invoice} />
        </div>
      </FillFormContainer>
    </FormContainer >
  );
